...
test('hierarchy - nesting', function(t) {
  ...
  t.deepEqual(map(enumMembers.static[0].path, 'name'), [
    'Parent',
    'enum',
    'Parent'
  ]);
  t.deepEqual(map(enumMembers.static[1].path, 'name'), [
    'Parent',
    'enum',
    'Child'
  ]);
...
test('hierarchy - object prototype member names', function(t) {
  ...
  t.deepEqual(map(comments[0].members.static[0].members.instance, 'name'), [
    'hasOwnProperty',
    'otherMethod'
  ]);
