...
function mergePackage(config/*: Object */)/*: Promise<Object> */ {
  ...
  return readPkgUp()
    .then(pkg =>
      _.defaults({}, _.mapKeys(_.pick(pkg.pkg, ['name', 'homepage', 'version']),
        (val, key) => `project-${key}`), config))
...
function mergeConfigFile(config)/*: Promise<Object> */ {
  ...
  if (config && typeof config.config === 'string') {
    ...
    return pify(fs).readFile(absFilePath, 'utf8')
      .then(rawFile => {
        if (ext === '.json') {
