...
var parse = require('../../lib/parsers/javascript'), hierarchy = require('../../lib/hierarchy');
...
it('hierarchy', function () {
  ...
  expect(map(classMembers.static[0].path, 'name')).toEqual(['Class', 'isClass']);
  expect(map(classMembers.instance[0].path, 'name')).toEqual(['Class', 'getFoo']);
  expect(map(classMembers.events[0].path, 'name')).toEqual(['Class', 'event']);
});

it('hierarchy - nesting', function () {
  var comments = evaluate(function () {
    /**
     * @name Parent
     * @class
  ...
  expect(map(enumMembers.static[0].path, 'name')).toEqual(['Parent', 'enum', 'Parent']);
  expect(map(enumMembers.static[1].path, 'name')).toEqual(['Parent', 'enum', 'Child']);
});

it('hierarchy - multisignature', function () {
  var comments = evaluate(function () {
    /**
     * @name Parent
     * @class
...
it('hierarchy - missing memberof', function () {
...
it('hierarchy - anonymous', function () {
...
it('hierarchy - object prototype member names', function () {
  var comments = evaluate(function () {
    /**
     * @name should
     * @function
  ...
  expect(map(comments[0].members.static[0].members.instance, 'name')).toEqual([ 'hasOwnProperty', 'otherMethod' ]);
});
