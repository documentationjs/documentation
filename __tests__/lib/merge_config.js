...
var path = require('path'), _ = require('lodash'), mergeConfig = require('../../lib/merge_config');

it('bad config', function (done) {
  mergeConfig({ config: 'DOES-NOT-EXIST' })
    .catch(err => {
...
it('nc(mergeConfig)', function (done) {

  // Omit configuration from output, for simplicity
  var nc = _.curryRight(_.omit, 2)(['config', 'no-package', 'parseExtension', 'project-homepage',
  ...
  Promise.all([
    ...
    { passThrough: true, config: path.join(__dirname, '../config_fixture/config.json') },
