 ...
 * @param {Array<Function>} elements - Pipeline elements. Each is a function that accepts
 ...
 */
function expandInputs(indexes /*: string|Array<string> */, config /*: DocumentationConfig */) {
...
function buildInternal(inputsAndConfig) {
  ...
  if (!config.dangerousCustomInference) {
    config.dangerousCustomInference = [];
  }
  ...
  var buildPipeline = pipeline(
    [
      ...
      .concat(config.dangerousCustomInference)
      .concat([(nest, config.github && github, garbageCollect)])
  );
 ...
 * @param {Array<Function>} [args.dangerousCustomInference] experimental feature:
 * specify an array of methods that run inference on source code. The methods
 * are given a comment and must return a comment.
